...
class Ymme(Database):
    ...
    def get_html(self, market):
        # filter market
        ymme_df = self.filter_market(self.database['YMME'], market)
        # column list to show
        column_list = ['Year', 'Make', 'Model', 'Engine']
        return ymme_df[column_list].sample(10).to_html(index=False)
...
class BatteryProcedure(Database):
    ...
    def __init__(self, path):
        ...
        self.vehicle_list_shuffle = None
        self.lang_filter = 'Description - English'

    # print 10 random samples
    def get_html(self, market, language=None, vehicle_change_flag = True):
        html = ''
        case_count = 0

        if language != None:
            self.lang_filter = 'Description - {}'.format(language.capitalize())


        # get vehicle list
        vehicle_list = self.database['YMME']

        # shuffle vehicle list
        if vehicle_change_flag == True:
            self.vehicle_list_shuffle = vehicle_list.sample(len(vehicle_list))

        # get all sets of procedure types and put into a df
        column_get_list = ['General Information', 'Before Battery Disconnection', 'Before Battery Connection', 'After Battery Connection']
        temp_df = vehicle_list
        temp_df = self.filter_market(vehicle_list, market)
        temp_df = temp_df[column_get_list] # get the set of procedure types
        temp_df = temp_df.drop_duplicates(subset=None, keep='first', inplace=False) # drop dups
        procedure_types_set_df = temp_df.sort_values(by=column_get_list) # sort

        # get procedure
        procedure_df = self.database['Procedure Type']

        # iterate procedure set and print html
        for index, row in procedure_types_set_df.iterrows():
            case_count += 1
            column_get_list = ['Year', 'Make', 'Model', 'Engine','General Information','Before Battery Disconnection','Before Battery Connection','After Battery Connection']

            temp_df = self.vehicle_list_shuffle
            ...
            html += '<b>' + 'CASE ' + str(case_count) + '</b>' + '<br>' 
            html += '<ul>'
            html += vehicle_display_list[column_get_list].head(3 if len(vehicle_display_list) > 3 else len(vehicle_display_list)).to_html(index=False)
            html += '<br>'*2
            html += procedure_df[procedure_df['Procedure Type'].isin(list(row))].to_html(index=False)
            #html += procedure_df[procedure_df['Procedure Type'].isin(list(row))][['Procedure Type', self.lang_filter]].to_html(index=False)
            html += '</ul>'
            html += '<br>'*2
...
class VinDecode(Database):
    ...
    def get_html(self, market):
        ...
        # column list to show
        column_list = ['Year', 'Make', 'Model', 'Engine', 'VIN', 'VIN Sim']
        
        # the database use to display as html
        self.database_display['main'] = vin_df[column_list]
