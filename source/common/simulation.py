    ...
    def create_obd2_ld(ld_inputs='default'):
        ...
        for pid in auto_pid_list:
            ...
            for itemid in itemid_list:
                ...
                itemname = item_df.loc[itemid,'ItemNameTracking']
...
class FordSimulation:
    ...
    def create_livedata(self, path):
        ...
        item_df = ford_ld.sheets['Item ID'].set_index('ItemID')


        profiles = ford_ld.get_profiles(year = year, 
                                        make= make, 
                                        model = model, 
                                        engine = engine,
                                        system = system)
        ...
        items = ford_ld.get_items(profiles)
        print('----------- items')
        print(items)

        groups = ford_ld.get_item_groups(items)
        ...
        for group in groups:
            ...
            for itemid in group:
                ...
                print(itemid)
                print(item_df.loc[itemid,'ItemNameTracking'])


                '''
                iter all item having the same PID request cmd
                '''
                try:
                    raw_value = ford_ld.get_random_raw_value(itemid)
                except Exception as e:
                    print('this ' + itemid + ' needed to check')
                    continue
                
                display_value = ford_ld.raw_to_readable(itemid, raw_value)

                itemname = item_df.loc[itemid,'ItemNameTracking']
                itemdes = item_df.loc[itemid,'ItemDescription']

                req_cmd = ford_ld.request_command(itemid)
                res_cmd = ford_ld.response_command(itemid, raw_value)

                print()
                print('request: ', req_cmd)
                print('response: ', res_cmd)
                ...
                simdata += ('//NOTE: "{}" -- "{}"\n' +
                            '//NOTE: PID: {} --- Value: {}\n' +
                            '//NOTE: \n').format(itemid,itemdes,req_cmd,display_value)
            

            print()
            print('request: ', req_cmd)
            print('res_concat_cmd: ', res_concat_cmd)
            req_split = req_cmd.split()
            res_split = res_concat_cmd.split()

            simdata += put_data_can(cmdtype='req', addr='00 00 07 xx', data=req_split) + '\n'
            simdata += put_data_can(cmdtype='res', addr='00 00 07 +08', data=res_split) + '\n'  + '\n'
...
class MazdaSimulation:
    ...
    def create_dtc(self):
        ...
        fordnws = MazdaNWS(mazda_nws_fp)
        forddtc = MazdaDTC(mazda_dtc_fp)
        ...
        dtc_sub_df = forddtc.sheets['Mazda-Sub']
        dtc_sub_df = dtc_sub_df[~dtc_sub_df['SubCodeDescription-English'].isna()]
        ...
        for index, row in ymme_df.iterrows():
            ...
            if type(protocol) != str: # The case when there are multiple same profile in profile sheet
