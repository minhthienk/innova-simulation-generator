...
class Database:
    ...
    def filter_ymme(cls,
        ...
        try:
            df = df[df['System'].isin([str(system), system, 'N/A', ''])] if system != None else df
        except KeyError as e:
            print('skip filtering Transmission')  
...
class Obd2_Monitors(Database):
    ...
    def get_monitors(self, engine_type):
        '''get a dictionary with keys are Monitor Abbreviation adn Monitor name
        '''
        df = self.sheets['Monitors']
        df = df[df['Engine Type']==engine_type]
        return {key:value for key,value in zip(df['Monitor'].to_list(),df['Description'].to_list())}
...
class LiveData(Database):
    ...
    def get_profiles(self,
        ...
        df = Database.filter_ymme(df, year, make, model, engine, trim, transmission, system)
...
#       print('\n\nprofile list: \n', profiles)
    ...
    def get_item_groups(self, items):
        ...
        for cmd in cmds_unique:
            temp_list = []
            for item in items:
                print('get_item_groups of cmd: ', cmd, ' --- item: ', item)
